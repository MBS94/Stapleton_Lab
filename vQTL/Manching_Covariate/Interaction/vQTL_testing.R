...
test_sov <- scanonevar(cross = fr,
                        mean.formula = Height ~ Env + mean.QTL.add + mean.QTL.dom + Env * (mean.QTL.add + mean.QTL.dom),
                        var.formula = ~ Env + var.QTL.add + var.QTL.dom + Env * (var.QTL.add + var.QTL.dom),
                        return.covar.effects = TRUE)
print("after scanonevar")

# Writing the result of the interactive scanonevar for later use
write_rds(test_sov, "test_sov.rds", compress = "xz")


# Writing out the results of the two 
write.csv(test_sov$result, file = "test_sov_result.csv")
