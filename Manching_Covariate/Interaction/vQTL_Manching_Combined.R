...
outv <- scanonevar(cross = dat,
                    mean.formula = Height ~ env + mean.QTL.add,
                    var.formula = ~ env + var.QTL.add, return.covar.effects = TRUE)
...
write.csv(outvdf, file = "2018_12_21_ManchingCovarCombined_vQTL.csv")
