---
title: "2018-08-02 Stapleton qPCR"
author: "Michael Byrd"
...
```{r}
hex <- read.csv(file = "2018_8_2_plate.csv")
```
...
# temp = t(cbind(c("cpD1","cpD2"), eff.frame))
# out = t(cbind(t(hex[1:3,]),temp))
# write.csv(out, file= "2018_8_2_plate_qPCR_output.csv")
# ```
```{r}
eff.frame = as.data.frame(rbind(hcpd1,hcpd2))
colnames(eff.frame) = colnames(hex)[-c(1)]; rownames(eff.frame) = c("cpD1","cpD2")
frameT = (cbind(c("cpD1","cpD2"), eff.frame))
hexT <- hex[]
temp = t(cbind(t(hexT[1:3, ]), t(frameT)))

# transposed_temp_hex = t(hex[-c(1,104,232,233)])
# out = t(cbind(t(temp_hex[1:3,]),temp))
write.csv(temp, file= "2018_08_02_plate_qPCR_output_2019_05_19.csv")
```