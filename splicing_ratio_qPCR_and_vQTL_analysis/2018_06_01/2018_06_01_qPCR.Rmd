...
```{r}
hex <- read.csv(file = "2018_6_1_plate.csv")
```
...
colnames(eff.frame) = colnames(hex)[-c(1,151,275,276)]; rownames(eff.frame) = c("cpD1","cpD2")
frameT = (cbind(c("cpD1","cpD2"), eff.frame))
hexT <- hex[-c(151,275,276)]
temp = t(cbind(t(hexT[1:3, ]), t(frameT)))

# transposed_temp_hex = t(hex[-c(1,104,232,233)])
# out = t(cbind(t(temp_hex[1:3,]),temp))
write.csv(temp, file= "2018_06_01_plate_qPCR_output_2019_04_04.csv")
```