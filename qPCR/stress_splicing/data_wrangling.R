...
bad <- which(!data$sampleID_Plus)
data = data[-bad,]
data = data[order(data$starting_quantity),]
test1 = c()
allP = c()
startq = c()

for(i in 1:length(data$starting_quantity)){
  sq <- data$starting_quantity[i]
  if(i %% 6 == 1){
  ...
  if(strcmp(val, "all_products")){
    allP <- c(allP, data$cpD1[i])
  }
}
d = as.data.frame(startq)
d = cbind(d, test1, allP)
test1Model = lm(d$test1~log10(d$startq))
allPModel = lm(d$allP~log10(d$startq))
write.csv(d, file="calib_data.csv")
